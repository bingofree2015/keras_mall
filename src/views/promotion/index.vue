...
<template>
    <div class="page-container">
                    ...
                    <el-form-item>
                        <el-input placeholder="请输入内容" v-model="filters.value">
                            ...
                            <ext-button :label="$t('action.search')" @click="queryForPaginatedList()"
                                icon="el-icon-ali-chazhaobiaodanliebiao" perms="promotion:view" slot="append"
                                type="primary" />
                        </el-input>
                    </el-form-item>
                    <el-form-item>
                        <ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add"
                            perms="promotion:add" type="primary" />
                    </el-form-item>
                    <el-form-item>
                        <el-button-group>
        ...
        <!--表格内容栏-->
        <ext-table aliasName="promotion" :batchDelete="batchDelete" :columns="columns" :count="paginated.attrs.count"
            :data="paginated.list" :operations="operations" :operationWidth="operationWidth"
    ...
    data () {
        return {
            ...
            columns: [
                ...
                { prop: 'type', label: '类型', minWidth: 80, formatter: this.env.columnFormatter },
                { prop: 'state', label: '状态', minWidth: 100, align: 'center', formatter: this.env.columnFormatter },
                { prop: 'exclusive', label: '排他', minWidth: 100, align: 'center', formatter: this.env.columnFormatter },
                { prop: 'startTime', label: '开始时间', minWidth: 130, formatter: this.env.formatDateTime },
                { prop: 'endTime', label: '结束时间', minWidth: 130, formatter: this.env.formatDateTime }
            ],
            operations: [
                {
                    ...
                    // type: 'primary',            // 按钮类型
                    func: (row) => {
                        this.$router.push({ path: '/promotion/edit', query: { id: row.id, isCreating: false } })
                    }
                }, {
                    label: 'action.delete',
