                    ...
                    <el-form-item>
                        <el-input placeholder="请输入内容" v-model="filters.value">
                            ...
                            <ext-button :label="$t('action.search')" @click="queryForPaginatedList()"
                                icon="el-icon-ali-chazhaobiaodanliebiao" perms="preference:logistics:view" slot="append"
                                type="primary" />
                        </el-input>
                    </el-form-item>
                    <el-form-item>
                        <ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add"
                            perms="preference:logistics:add" type="primary" />
                    </el-form-item>
                    <el-form-item>
                        <el-button-group>
        ...
        <el-dialog :close-on-click-modal="false" :title="isCreating?'新增':'编辑'" :visible.sync="editDialogVisible"
            ...
            <el-form :model="formData" :rules="formDataRules" :size="normalSize" label-width="80px" ref="formData">
                <el-form-item label="公司名称" prop="logiName">
                    <el-input v-model="formData.logiName"></el-input>
                </el-form-item>
                <el-form-item label="公司编码" prop="logiCode">
                    <el-input v-model="formData.logiCode"></el-input>
                </el-form-item>
            ...
            this.$refs.formData.validate((valid) => {
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then(async () => {
                        ...
                        if (_result.succeed === 1 && _result.code === 200) {
                            const _logistics = this.paginated.list.find(v => v.id === _result.data.id)
                            if (!_logistics) {
