                    ...
                    <el-form-item>
                        <el-input placeholder="请输入内容" v-model="filters.value">
                            ...
                            <ext-button :label="$t('action.search')" @click="queryForPaginatedList()"
                                icon="el-icon-ali-chazhaobiaodanliebiao" perms="marketing:article:view" slot="append"
                                type="primary" />
                        </el-input>
                    </el-form-item>
                    <el-form-item>
                        <ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add"
                            perms="marketing:article:add" type="primary" />
                    </el-form-item>
                    <el-form-item>
                        <el-button-group>
...
export default {
    ...
    data () {
        return {
            ...
            columns: [
                ...
                { prop: 'attachment.path', label: '封面图', minWidth: 80, propType: 'image', align: 'center' },
                { prop: 'articleType.typeName', label: '分类', minWidth: 80 },
                { prop: 'isPub', label: '状态', minWidth: 80 },
                { prop: 'sort', label: '排序', minWidth: 70, align: 'center' },
                { prop: 'updatedAt', label: '更新时间', minWidth: 130, formatter: this.env.formatDateTime },
            ...
            operations: [
                {
                    ...
                    // type: 'primary',            // 按钮类型
                    func: (row) => {
                        this.$router.push({ path: '/marketing/article/edit', query: { id: row.id, isCreating: false } })
                    }
                }, {
                    label: 'action.delete',
    ...
    methods: {
        ...
        // 显示新增界面
        handleAdd () {
            this.$router.push({ path: '/marketing/article/edit', query: { id: 0, isCreating: true } })
        }
    },
    mounted () { }
}
